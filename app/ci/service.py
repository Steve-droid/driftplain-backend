...
def _exit_case(*, pass_msg: str, fail_msg: str, nothing_verb: str) -> str:
    return f"""        case "$AGENT_RC" in
          0)   echo "Driftplain: {pass_msg}" ;;
          1)   echo "Driftplain: {fail_msg} — failing the stage." ;;
          2)   echo "Driftplain: unparseable model output — NOT a pass." ;;
          3)   echo "Driftplain: the model REFUSED. Nothing was {nothing_verb}. NOT a pass." ;;
          4)   echo "Driftplain: config / credential / API error (see above) — NOT a pass." ;;
          124) echo "Driftplain: a run ceiling aborted the agent — NOT a pass." ;;
          *)   echo "Driftplain: unexpected agent exit $AGENT_RC — treating as failure." ;;
        esac
        exit $AGENT_RC"""
...
// project's task, model and review preferences from Driftplain at run time and posts
// the run itself — change them in the app, not here.
stage('Driftplain AI Review') {{
  agent any
  environment {{
...
// READ-ONLY with the model Driftplain configured for this project, posts the run itself
